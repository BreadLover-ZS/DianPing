...
代码层面的可靠性闭环已完成（预留账本、Outbox、失败决策、持久化回滚、双向对账、DLQ 失败记录与处置 Service、订单状态查询）。上线迁移步骤 1-2 已于 2026-08-21 在远程环境执行并验证：事件表完成备份，两个迁移脚本已应用，新增列、任务索引和三张新表均已确认；事件表当时为 0 行，无旧 FAILED 事件需要迁移。仍需完成真实 RabbitMQ 故障注入演练（规格 18.4 节）、跨存储崩溃窗口演练（18.5 节）、并发压测，以及随真实流量上线执行迁移步骤 3-8；RBAC 完成后才能开放失败处置 Controller。消费者默认关闭，需真实连通性验收后启用。

### 6.1 RabbitMQ 秒杀改造面试追问（可靠性闭环已完成）

> **事实边界：** 已完成（代码 + 单元测试）：交换机/队列/死信拓扑、JSON 持久化消息、六 Key 预留账本 Lua（含 orderId 反向索引，同 `{voucherId}` 槽）、事件表 Outbox（CAS + 租约）、发布尝试证据表、统一失败决策服务、异常三分类、监听前 ErrorHandler、DLQ 消费者、持久化回滚任务、双层对账任务（7 天快速扫描 + 每小时全量分页兜底）、库存安全初始化与缺失扫描、失败记录 + 审计 + 处置 Service、订单状态查询接口（新旧两版）。
>
> **真实环境证据：** 2026-08-21 已获授权在远程 `dish_review` 环境执行上线迁移步骤 1-2：备份 `tb_seckill_order_event`，应用两个迁移脚本，并确认 9 个新列、组合索引 `idx_seckill_order_event_task` 及 `tb_seckill_publish_attempt`、`tb_seckill_failure_case`、`tb_seckill_failure_audit` 三张新表；事件表当时为 0 行，无旧 FAILED 事件需要转 MANUAL_REVIEW。
>
> **未完成：** 远端 RabbitMQ 可达性验收、真实并发与故障注入演练、跨存储崩溃窗口演练、随真实流量上线执行迁移步骤 3-8，以及 RBAC 与失败处置 Controller。
...
- [x] 执行上线迁移步骤 1-2：备份事件表、应用两个迁移脚本并验证新增列、索引和三张新表（2026-08-21 远程环境）。
- [ ] 随真实流量上线按规格 17.1 节完成步骤 3-8：暂停入口、存量核对、Redis 用户集合补录、兼容部署和小流量验证。
...
| 2026-08-21 | 秒杀可靠性闭环改造 | 按规格 10 完成阶段 1-6：预留账本、Outbox、失败决策、异常三分类、DLQ 闭环、持久化回滚、双向对账、库存安全初始化、订单状态查询、失败处置 Service；新增 R10-R18 面试题；三轮验收修复关键状态机和前后端联动；最终调整人工移交 Lua 为“先写目标、后删源入口”，避免运行时错误留下不完整迁移 | 185 个单元测试全部通过；上线迁移步骤 1-2 已在远程环境执行并验证，事件表当时为空；步骤 3-8、真实 RabbitMQ 故障演练与并发压测尚未执行 |
